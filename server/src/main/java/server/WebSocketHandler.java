...
public class WebSocketHandler implements WsConnectHandler, WsMessageHandler, WsCloseHandler{
    ...
    public void handleConnectCommand(WsContext ctx, UserGameCommand command) {
        try {
            ...
            gameSessions.computeIfAbsent(gameID, s -> ConcurrentHashMap.newKeySet()).add(ctx);
    ...
    public void handleMove(WsContext ctx, MakeMoveCommand command) {
        try {
            ...
            if (move.getPromotionPiece() == null) {
                ...
                broadcastToOthers(gameID, ctx, username + " (" + color + ") "
                        + convertPosToCor(move.getStartPosition()) + " to "
                        + convertPosToCor(move.getEndPosition()) + " Promotion: " + move.getPromotionPiece());
            }
